...
def processInnerLoc(req):
	result = req.get("result")
	parameters = result.get("parameters")
	inner = parameters.get("inner_loc")
	return inner

def processPurpose(req):
	result = req.get("result")
	parameters = result.get("parameters")
	inner = parameters.get("buy_rent")
	return inner
...
def makeWebhookResult(data):
	...
	while (i <length):
		...
		if innerloc_check==None:
			areasub="\nThere is no such area in "+city_names+".Here are some properties in"+city_names+"."
         ...
         "attachment": {
           "type": "template",
            "payload": {
              ...
              "subtitle":"Price: Rs."+str(row_price[0])+areasub,
